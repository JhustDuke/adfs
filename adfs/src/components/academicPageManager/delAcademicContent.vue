<template>
	<div class="row g-3 py-4">
		...
		<div
			...
			<p class="mt-2 text-muted">Loading academic cards...</p>
		</div>
...
<script setup lang="ts">
	...
	const cards = ref<AcademicCardInterface[]>([]);
	const isFetching = ref(true);
	const fetchError = ref(false);

	const showDeleteBox = ref(false);
	const selectedCard = ref<AcademicCardInterface | null>(null);
	const successMessage = ref("");

	/**
	 * fetch academic cards
	 */
	 ...
	 * open delete modal
	 */
	const openDeleteBox = function (card: AcademicCardInterface): void {
	...
	const handleDelete = async function (): Promise<void> {
		...
		try {
			const res = await fetch("/api/academic/deleteAcademicCard", {
				method: "DELETE",
				headers: {
